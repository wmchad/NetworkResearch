...
for ( b in betas ) {
  for ( k in ldims ) {
    cpResult[[index]] <- list(beta=b, ldim=k,
    ...
    print(cpResult[[index]]$result$cp);
    print("");
    print("");
    index <- index + 1;
  }
...
save(cpResult, file="bombingCpmf.rdata");
...
index <- 1;
for ( i in 1:10 ) {
  cpResult[[i]] <- list(beta=beta, ldim=2,
                        result=SymmetricMF.Changepoint.PELT(
                          callData.adj, 2, 0, beta,
                          maxiter=maxiter,
                          epsilonInter=epsilonInter,
                          epsilonFinal=epsilonFinal,
                          verbose=FALSE));
  ...
  print(cpResult[[i]]$result$cp);
  print("");
  print("");
}

cpResult.test1Setting <- cpResult;

setwd("c:/Code/NetworkResearch/R/Test/CPMF/Results");
save(cpResult.test1Setting, file="cpResult.test1Setting.rdata");
